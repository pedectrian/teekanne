...
function onepage_optionsframework_add_admin() {
    global $query_string;

    $themename = onepage_get_option('of_themename');
    $shortname = onepage_get_option('of_shortname');

    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework') {
        if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
            $options = onepage_get_option('of_template');
            onepage_reset_options($options, 'optionsframework');
    ...
    $of_page = add_theme_page($themename, 'Theme Options', 'edit_theme_options', 'optionsframework', 'onepage_optionsframework_options_page', 'div');

    // Add framework functionaily to the head individually
    add_action("admin_print_scripts-$of_page", 'onepage_load_only');
}

add_action('admin_menu', 'onepage_optionsframework_add_admin');
/* ----------------------------------------------------------------------------------- */
/* Options Framework Reset Function - of_reset_options */
/* ----------------------------------------------------------------------------------- */

function onepage_reset_options($options, $page = '') {
    global $wpdb;
    $count = 0;
    ...
    if ($page == 'optionsframework') {
        onepage_delete_option('of_options');
    }
}
...
function onepage_optionsframework_options_page() {
    $options = onepage_get_option('of_template');
    $themename = onepage_get_option('of_themename');
    ?>
    ...
    $return = onepage_optionsframework_machine($options);
    ?>
            <div id="main">
                <div id="of-nav">
                    <ul>
        ...
        foreach ($options as $option) {
            if ($option['type'] == 'color' OR $option['type'] == 'typography' OR $option['type'] == 'border') {
                if ($option['type'] == 'typography' OR $option['type'] == 'border') {
                    $option_id = $option['id'];
                    $temp_color = onepage_get_option($option_id);
                    $option_id = $option['id'] . '_color';
                    $color = $temp_color['color'];
                } else {
                    $option_id = $option['id'];
                    $color = onepage_get_option($option_id);
                }
                ?>
...
/* ----------------------------------------------------------------------------------- */
/* Ajax Save Action - onepage_ajax_callback */
/* ----------------------------------------------------------------------------------- */
add_action('wp_ajax_of_ajax_post_action', 'onepage_ajax_callback');

function onepage_ajax_callback() {
    ...
    if ($save_type == 'upload') {
        ...
        onepage_update_option($clickedID, $uploaded_file['url']);
        ...
        foreach ($options as $option_array) {
            $id = $option_array['id'];
            $old_value = onepage_get_option($id);
            $new_value = '';
                ...
                if (is_array($type)) {
                    foreach ($type as $array) {
                        if ($array['type'] == 'text') {
                            ...
                            onepage_update_option($id, stripslashes($new_value));
                        }
                    }
                } elseif ($new_value == '' && $type == 'checkbox') { // Checkbox Save
                    onepage_update_option($id, 'false');
                } elseif ($new_value == 'true' && $type == 'checkbox') { // Checkbox Save
                    onepage_update_option($id, 'true');
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if (!isset($output[$multicheck_id])) {
                            onepage_update_option($multicheck_id, 'false');
                        } else {
                            onepage_update_option($multicheck_id, 'true');
                        }
                    }
                    ...
                    onepage_update_option($id, $typography_array);
                } elseif ($type == 'border') {
                    ...
                    onepage_update_option($id, $border_array);
...
function onepage_optionsframework_machine($options) {
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'text':
                $val = $value['std'];
                $std = onepage_get_option($value['id']);
                if ($std != "") {
                    $val = $std;
                }
            ...
            case 'select':
                ...
                $select_value = onepage_get_option($value['id']);
            ...
            case "upload":
                $value['std'] = '';
                if (isset($value['std'])) {
                    $output .= onepage_optionsframework_uploader_function($value['id'], $value['std'], null);
                }
                break;
            case "upload_min":

                $output .= onepage_optionsframework_uploader_function($value['id'], $value['std'], 'min');

                break;
            case "color":
                $val = $value['std'];
                $stored = onepage_get_option($value['id']);
                if ($stored != "") {
                    $val = $stored;
                }
            ...
            case "typography":
                ...
                $default = $value['std'];
                $typography_stored = onepage_get_option($value['id']);
            ...
            case "border":
                ...
                $default = $value['std'];
                $border_stored = onepage_get_option($value['id']);
        ...
        if (is_array($value['type'])) {
            foreach ($value['type'] as $array) {
                $id = $array['id'];
                $std = $array['std'];
                $saved_std = onepage_get_option($id);
                if ($saved_std != $std) {
                    $std = $saved_std;
                }
                $meta = $array['meta'];
...
/* ----------------------------------------------------------------------------------- */
/* OptionsFramework Uploader - onepage_optionsframework_uploader_function */
/* ----------------------------------------------------------------------------------- */

function onepage_optionsframework_uploader_function($id, $std, $mod) {
    ...
    $uploader = '';
    $upload = onepage_get_option($id);
    if ($mod != 'min') {
        $val = $std;
        if (onepage_get_option($id) != "") {
            $val = onepage_get_option($id);
        }
